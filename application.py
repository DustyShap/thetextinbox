...
def index():
    ...
    for user in all_users:
        ...
        all_user_dict['text_count'] = user_total_texts(user.id)
        all_users_list.append(all_user_dict)
    return render_template('textline.html',all_users=all_users_list, all_messages=all_messages, is_admin=is_admin,total_texts=len(all_messages))


@app.route("/all_single_user_messages", methods=['POST'])
def all_single_user_messages():
    all_user_messages = get_all_user_messages(request.form['user_id_clicked'])
...
def search_for_user():
    ...
    for user in users:
        ...
        all_searched_users_dict['text_count'] = user_total_texts(user.id)
...
def sms():
    ...
    if user is None:
        user = User(phone_number=phone_number,display_name=default_name,location=location)
        db.session.add(user)
        db.session.commit()
    ...
    message = Message(message_body=message_body,message_timestamp=timestamp,message_user_id=user.id)
    if num_media > 0:
        media = request.form["MediaUrl0"]
        message.message_media_url = media
    db.session.add(message)
    db.session.commit()
    total_texts_from_user = user_total_texts(user.id)
    ...
    new_text = {
        ...
        "total_texts_from_user":total_texts_from_user,
    }
...
def update_name():
    ...
    user_to_update = User.query.filter_by(phone_number=number_to_update).first()
...
def get_all_messages():
    return db.session.query(Message,User).join(User, Message.message_user_id == User.id).order_by(Message.message_timestamp).all()
